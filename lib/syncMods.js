...
/**
 * Génère un nom obfusqué à partir du nom original du mod.
 * SHA256 du nom, tronqué à 12 caractères hex + .jar
 * @param {string} originalName
 * @returns {string}
 */
function obfuscateName(originalName) {
    return crypto.createHash('sha256').update(originalName).digest('hex').slice(0, 12) + '.jar';
}

/**
 * Synchronize local mods with server manifest.
 * Blocks launch if the server is unreachable.
 * Mod files are stored locally with obfuscated names.
 *
 ...
 */
async function syncMods(gameDir, baseUrl, sendProgress, options = {}) {
    ...
    // Server uses real names; locally we store with obfuscated names
    const serverMods = new Map(); // obfuscatedName → { serverName, sha256 }
    for (const mod of manifest.mods) {
        const localName = obfuscateName(mod.name);
        serverMods.set(localName, { serverName: mod.name, sha256: mod.sha256 });
    }

    // 2. Hash local mods (already obfuscated names on disk)
    //    When skipVerify is set (non-first instance), skip hashing to avoid
    //    blocking on Windows file locks — just check file presence by name.
    const localFiles = fs.existsSync(modsDir)
        ? fs.readdirSync(modsDir).filter(f => f.endsWith('.jar'))
        : [];

    if (options.skipVerify) {
        console.log('[syncMods] skipVerify mode — skipping hash check');
        // Trust existing files — the first instance already verified them
        const localSet = new Set(localFiles);
        const missing = [];
        for (const [localName, { serverName, sha256 }] of serverMods) {
            if (!localSet.has(localName)) {
                missing.push(localName);
            }
        }
        if (missing.length > 0) {
            console.log(`[syncMods] skipVerify: ${missing.length} mod(s) missing — deferring to first instance`);
        } else {
            console.log(`[syncMods] skipVerify: all ${serverMods.size} mods present — skipping hash check`);
        }
        sendProgress('done', 0, 0, null);
        const allowedMods = new Map();
        for (const [localName, { sha256 }] of serverMods) {
            allowedMods.set(localName, sha256);
        }
        return allowedMods;
    }
    ...
    // 3. Build sync plan (compare using obfuscated local names)
    const toDownload = [];
    const toDelete = [];
    const upToDate = [];

    for (const [localName, { serverName, sha256 }] of serverMods) {
        const localHash = localHashes.get(localName);
        if (localHash === sha256) {
            upToDate.push(localName);
        } else {
            toDownload.push({ localName, serverName, expectedHash: sha256 });
        }
    }
    ...
    //    URL uses the real server name, but saved locally with obfuscated name
    if (toDownload.length > 0) {
        for (let i = 0; i < toDownload.length; i++) {
            const { localName, serverName, expectedHash } = toDownload[i];
            const destPath = path.join(modsDir, localName);
            const url = `${baseUrl}/mods/${encodeURIComponent(serverName)}`;
            sendProgress('downloading', i + 1, toDownload.length, localName);
            console.log(`[syncMods] Downloading: ${serverName} → ${localName}`);
            ...
            for (let attempt = 0; attempt < 2; attempt++) {
                try {
                    ...
                    if (actualHash !== expectedHash) {
                        throw new Error(`Hash mismatch for ${serverName}: expected ${expectedHash}, got ${actualHash}`);
                    }
                    success = true;
                    break;
                } catch (err) {
                    console.error(`[syncMods] Attempt ${attempt + 1} failed for ${serverName}:`, err.message);
    ...
    // Return Map of obfuscatedName → sha256 for the mods guard
    const allowedMods = new Map();
    for (const [localName, { sha256 }] of serverMods) {
        allowedMods.set(localName, sha256);
    }
 ...
 */
function createModsGuard(modsDir, allowedMods, options = {}) {
    let watcher = null;
...
async function syncConfigs(gameDir, baseUrl, sendProgress) {
    ...
    for (const entry of serverConfigs) {
        ...
        if (fs.existsSync(localPath)) {
            try {
                console.log(`[syncConfigs] hashing config: ${entry.path} ...`);
    ...
    // 3. Delete local files not in the manifest — only within directories
    //    that the manifest manages (e.g. fancymenu/). User-created configs
    //    in other directories or at the root are left untouched.
    const serverPaths = new Set(serverConfigs.map(e => e.path));
    const managedDirs = new Set(serverConfigs.map(e => e.path.split('/')[0]).filter(d => d.includes('.') === false));
    const localFiles = walkDirRelative(configDir, configDir);
    const toDelete = localFiles.filter(f => {
        if (serverPaths.has(f)) return false;
        const topDir = f.split('/')[0];
        return managedDirs.has(topDir) && topDir !== f;
    });
