 ...
 * Only downloads missing or modified files — does NOT delete extra local configs.
 *
 * @param {string} gameDir  - e.g. %APPDATA%/.matcraft
 * @param {string} baseUrl  - e.g. https://matfaction.com/launcher
...
async function syncConfigs(gameDir, baseUrl, sendProgress) {
    ...
    const manifestUrl = `${baseUrl}/configs/manifest.json`;
    ...
    console.log(`[syncConfigs] Up to date: ${upToDate.length}, to download: ${toDownload.length}`);

    // 3. Download missing/modified configs
    if (toDownload.length > 0) {
        for (let i = 0; i < toDownload.length; i++) {
            const entry = toDownload[i];
            const destPath = path.join(configDir, entry.path);
            const url = `${baseUrl}/configs/${entry.path.split('/').map(encodeURIComponent).join('/')}`;
